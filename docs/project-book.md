...
- 纯函数、分支、for 循环 → 有向无环图（DAG）→ 按拓扑序串行执行。
- loop、recv → 带反馈环的图 → 循环迭代，节奏固定。
- flow、go、yield、select → 运行时动态图 → 节点并发激活，数据在边上流动。

代码结构决定图结构，图结构决定执行方式。不同的执行特征由拓扑自然产生，无需在代码中声明模式。
...
Core 的指针安全完全建立在数据流图上，不引入 borrow checker、Arena tag、RawRef 等独立概念层。编译器从图中推导每个指针的来源（provenance）和偏移，在解引用点自动验证。详见 `docs/pointer-model.md`。
...
引入 .corespec 文件。在理解函数签名和类型系统之后，进一步学习如何用前置条件、后置条件和循环不变量精确描述函数的行为约束。

### 第五阶段：系统编程

用同一门语言写内核、驱动、网络协议栈。之前每个阶段学到的概念都在：DAG 对应中断处理路径，带环静态图对应设备轮询，动态图对应进程调度器，规约对应安全关键的隔离性质。汇编层由 `.crasm` 统一汇编抽象层承担（跨平台统一指令集 + 无限虚拟寄存器 + 平台映射表，设计见 `docs/crasm.md`）。

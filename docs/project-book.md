...
**三层映射（2026-08-26）**：语义 → 图 → 格 → 二进制——**图负责表达计算**（关系空间），**格负责承载计算**（内存模型 = 中间存在空间），**二进制负责实现计算**（物理编码空间）。跨范式发生在图层，超图灵性属于图不属于格；格的价值 = 表达空间未被有限范式封顶——不同范式各找自己的格映射（见 `docs/memory-model-capability-lattice.md` §四）。

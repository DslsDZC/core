...
| 查找函数 | find_func | 类型方法判断 |
| 查找符号 | find_sym | 收集声明 |
| 分配类型 | alloc_type | 解析类型节点 |
| 获取类型类别 | get_type_kind | 获取类型名称 |
| 获取类型数据 | get_type_data | 获取类型名称 |
| 类型相等判断 | type_equal | 收集声明 |
| 检查错误 | check_error | 解析类型节点 |
| 定义符号 | def_sym | 收集声明 |
| 查找泛型符号 | find_gsym | 解析类型节点 |
| 读写全局 | r64 / w64 | 多个函数 |
| 驻留字符串获取 | istr_get | 类型方法判断 |
| 字符串驻留 | str_intern | 类型方法判断 |
| 整数转字符串 | int_str | 收集声明 |
...
| 函数信息访问器：AST 节点 | fi_ast_node | 收集声明 |
...
| 符号类别 | sym_kind | 查找泛型符号 |
| 符号类型 | sym_type | 查找泛型符号 |
| 符号节点 | sym_node | 收集声明 |
...
如果 调用 AST 访问器：类别（ast_kind）（node）等于 泛型应用表达式（EXPR_GENERIC_APPLY），那么：
    （泛型应用：例如 Box[整数（int）]）
    令 名称索引（name_idx） = 调用 第一子节点访问器（ast_a）（ast_a）（node）
    令 首参节点（first_arg_node） = 调用 第二子节点访问器（ast_b）（ast_b）（node）
    令 参数个数（arg_count） = 调用 第三子节点访问器（ast_c）（ast_c）（node）

    令 符号索引（si） = 调用 查找泛型符号（gsym）（find_gsym）（name_idx）
    如果 符号索引（si）小于 0 或 调用 符号类别（sym_kind）（si）不等于 符号类别：类型（SYM_TYPE），那么：
        调用 检查错误（check_error）（泛型类型未定义错误码（EC_N_GENERIC_TYPE），"泛型应用中的未定义类型"，调用 AST 访问器：行号（ast_line）（node），调用 AST 访问器：列号（ast_col）（节点（节点）））
        返回 类型表预分配：单元（TI_UNIT）
    ...
    调用 读写全局：写 64 位（w64）（泛型应用数据数组（g_gen_apply_data），数据起始索引（data_start） * 8，参数个数（arg_count））
    ...
    循环：
        ...
        调用 读写全局：写 64 位（w64）（泛型应用数据数组（g_gen_apply_data），（数据起始索引（data_start） + 1 + 参数索引（ai）） * 8，参数类型表索引（arg_ti））
        参数索引（ai） = 参数索引（ai） + 1
        参数节点（an） = 参数节点（an） + 1
...
7. 指针类型 *泛型参数（T）（非 非安全（unsafe） 块内）-> 指针类型（TYP_PTR）（inner, asp=0）
8. 指针类型 *泛型参数（T）（非安全（unsafe） 块内）-> 指针类型（TYP_PTR）（inner, asp=1）
...
循环：
    ...
    如果 调用 枚举信息访问器：名称（ei_name）（i）等于 名称索引（name_idx），那么：返回 索引（i）
    索引（i） = 索引（i） + 1
返回 -1
`
...
循环：
    ...
    如果 调用 读写全局：读 64 位（r64）（接口数组（g_ifaces），索引（i） * 接口信息大小（ESZ_IFACEINFO） + OFF_IF_NAME）等于 名称索引（name_ni），那么：返回 索引（i）
    索引（i） = 索引（i） + 1
返回 -1
`
...
### 作用
从类型表索引（ti）提取可读的类型名称（字符串驻留索引）。基础类型返回 "int"/"float"/"bool"/"string"/"unit"/"char" 的驻留索引；命名类型返回其名称字符串索引；泛型应用的命名基础类型返回其基础类型的名称；其他返回 -1。
...
如果 类别（k）等于 类型条目类别：基础（TYP_BASE），那么：
    ...
    如果 数据（d）等于 整数（TY_INT），那么：返回 调用 字符串驻留（str_intern）（"int"）
    如果 数据（d）等于 浮点数（TY_FLOAT），那么：返回 调用 字符串驻留（str_intern）（"float"）
    如果 数据（d）等于 布尔（TY_BOOL），那么：返回 调用 字符串驻留（str_intern）（"bool"）
    如果 数据（d）等于 字符串（TY_STRING），那么：返回 调用 字符串驻留（str_intern）（"string"）
    如果 数据（d）等于 单元（TY_UNIT），那么：返回 调用 字符串驻留（str_intern）（"unit"）
    如果 数据（d）等于 字符（TY_CHAR），那么：返回 调用 字符串驻留（str_intern）（"char"）
...
### 逻辑
`
令 方法个数（method_count） = 调用 读写全局：读 64 位（r64）（接口数组（g_ifaces），接口索引（iface_ii） * 接口信息大小（ESZ_IFACEINFO） + OFF_IF_METHOD_COUNT）
...
循环：
    ...
    令 方法名索引（method_ni） = 调用 读写全局：读 64 位（r64）（接口数组（g_ifaces），方法基址（mbase2） + OFF_IFM_NAME）
    ...
    如果 函数索引（fi2）大于等于 0，那么：
        令 接口参数个数（iface_pc） = 调用 读写全局：读 64 位（r64）（接口数组（g_ifaces），方法基址（mbase2） + OFF_IFM_PARAM_COUNT）
        如果 调用 函数信息访问器：参数计数（fi_param_count）（fi2）不等于 接口参数个数（iface_pc），那么：返回 假
        令 接口返回类型（iface_rt） = 调用 读写全局：读 64 位（r64）（接口数组（g_ifaces），方法基址（mbase2） + OFF_IFM_RET_TI）
...
循环：
    ...
    如果 泛型个数（gc）大于 0，那么：
        ...
        循环：
            ...
            调用 读写全局：写 64 位（w64）（泛型参数数组（g_gen_params），泛型参数计数（g_gen_param_count） * 8，泛型名索引（gname_ni））
            调用 读写全局：写 64 位（w64）（泛型参数数组（g_gen_params），（泛型参数计数（g_gen_param_count） + 1） * 8，泛型类型表索引（g_ti））
...
循环：
    ...
    令 名称索引（name_idx） = 调用 读写全局：读 64 位（r64）（接口数组（g_ifaces），索引（i） * 接口信息大小（ESZ_IFACEINFO） + OFF_IF_NAME）
    令 类型索引（type_idx） = 调用 分配类型（alloc_type）（类型条目类别：命名（TYP_NAMED），名称索引（name_idx），0）
    调用 定义符号（def_sym）（名称索引（name_idx），符号类别：类型（SYM_TYPE），类型索引（type_idx），-1）
    索引（i） = 索引（i） + 1
...
循环：
    ...
    如果 调用 枚举信息访问器：名称（ei_name）（i）等于 调用 字符串驻留（str_intern）（"Option"），那么：
        令 是否找到选项（option_found） = 1
    索引（i） = 索引（i） + 1

如果 是否找到选项（option_found）等于 0，那么：
    （自动将 可选类型（Option） 注册为一个泛型内建类型）
    令 选项名索引（option_name_idx） = 调用 字符串驻留（str_intern）（"Option"）
...
循环：
    ...
    令 名称索引（name_idx） = 调用 读写全局：读 64 位（r64）（类型别名数组（g_type_aliases），索引（i） * 16）
    令 类型节点（type_node） = 调用 读写全局：读 64 位（r64）（类型别名数组（g_type_aliases），索引（i） * 16 + 8）
...
循环：
    ...
    如果 热补丁版本号（hotpatch_ver）大于 0，那么：
        ...
        令 加工名（mangled_name） = 函数名字符串（fn_name_str） + "v" + 调用 整数转字符串（int_str）（hotpatch_ver）
...
循环：
    ...
    如果 调用 AST 访问器：类别（ast_kind）（ei）等于 外部函数表达式（EXPR_EXTERN），那么：
        令 名称索引（name_ni） = 调用 第一子节点访问器（ast_a）（ast_a）（ei）
        令 首参（first_param） = 调用 第二子节点访问器（ast_b）（ast_b）（ei）
        令 参数个数（param_count） = 调用 第三子节点访问器（ast_c）（ast_c）（ei）
...
循环：
    ...
    令 节点（node） = 调用 读写全局：读 64 位（r64）（全局声明数组（g_global_lets），索引（i） * 8）
    令 名称索引（name_idx） = 调用 第一子节点访问器（ast_a）（ast_a）（node）   （变量声明表达式（EXPR_LET）：变量甲 = 名称索引）
    令 类型节点（type_node） = 调用 第二子节点访问器（ast_b）（ast_b）（node）  （变量声明表达式（EXPR_LET）：第二子节点（变量乙） = 类型节点，-1 表示无）
...
循环：
    ...
    令 别名索引（alias_ni） = 调用 读写全局：读 64 位（r64）（模块数组（g_mods），模块索引（mi） * 24）
    令 文件 ID 索引（fileid_ni） = 调用 读写全局：读 64 位（r64）（模块数组（g_mods），模块索引（mi） * 24 + 8）
...
循环：
    ...
    令 模块路径名（mpn） = 调用 读写全局：读 64 位（r64）（模块路径名数组（g_mod_path_names），路径索引（pi） * 8）
...
如果 文件计数（g_file_count）大于 0，那么：令 主文件 ID 索引（main_fni） = 调用 读写全局：读 64 位（r64）（文件数组（g_files），0）
...
循环：
    ...
    如果 函数行号（fn_line）大于 0 且 函数行号（fn_line）小于等于 行计数（g_line_count），那么：
        令 文件 ID 索引（fileid_ni） = 调用 读写全局：读 64 位（r64）（行文件 ID 数组（g_line_fileid），（函数行号（fn_line） - 1） * 8）
        如果 文件 ID 索引（fileid_ni）不等于 主文件 ID 索引（main_fni）且 文件 ID 索引（fileid_ni）不等于 0 且 行计数（g_line_count）大于 0，那么：
            ...
            调用 读写全局：写 64 位（w64）（模块函数文件 ID 数组（g_mod_func_fileids），模块函数计数（g_mod_func_count） * 8，文件 ID 索引（fileid_ni））
            调用 读写全局：写 64 位（w64）（模块函数名数组（g_mod_func_names），模块函数计数（g_mod_func_count） * 8，调用 函数信息访问器：名称（fi_name）（fi））
            令 函数符号索引（fn_si） = 调用 查找符号（find_sym）（调用 函数信息访问器：名称（fi_name）（fi））
            如果 函数符号索引（fn_si）大于等于 0，那么：
                调用 读写全局：写 64 位（w64）（模块函数类型索引数组（g_mod_func_tis），模块函数计数（g_mod_func_count） * 8，调用 符号类型（sym_type）（fn_si））

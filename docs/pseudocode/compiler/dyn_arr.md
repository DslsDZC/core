...
| 函数信息访问器：ast节点 | fi_ast_node | 函数信息访问器族 |
...
2. 负数（字节3（b3） 大于等于 128）通过 字节3（字节3） - 256 进行符号扩展
...
2. 逐字节拷贝，源和目标缓冲区可以重叠吗（取决于调用方的用法，通常用于扩容后旧缓冲区到新缓冲区的拷贝）
...
| 扩展ast（grow_ast） | g_ast（AST 节点数组） | g_ast_cap（AST 节点容量） | ESZ_ASTNODE （72） | 128 | 128 |
...
#### 逻辑
    ...
    写 64 位（w64）（g_ast, 新节点索引 * 72 + OFF_AS_KIND（0）, 种类（kind））
...
| 函数信息访问器：ast节点（fi_ast_node） | 读 | 返回 读 64 位（g_funcs, n * 208 + OFF_FI_AST_NODE（152）） |
...
### 函数 哈希值（hash）字节（hash_bytes）
签名：`函数 哈希值（hash）字节（hash_bytes）（数据指针（data）：字符串，长度（len）：整数）-> 整数`
#### 作用
...
#### 逻辑
    ...
    循环：
        ...
        如果 字符串相等比较（str_eq）（字符串指针加载（load_str_ptr）（g_strs, 槽中字符串索引 * 8）, 状态（s））不等于 0，那么：
            返回 槽中字符串索引（已存在，直接返回）
        令 探测位置 = （探测位置 + 1） 模 当前容量（线性探测下一个槽位）

    （未找到：将新字符串追加到 g_strs 数组）
    扩展全局字符串数组（grow_g_strs）（g_str_count + 1）
    字符串指针存储（store_str_ptr）（g_strs, g_str_count * 8, 状态（s））
...
#### 作用
按索引从驻留表获取字符串指针。越界（索引小于 0 或大于等于 g_str_count）返回空串。
#### 逻辑
...
#### 作用
按驻留表索引和字符位置读取一个字节。越界返回 0。
#### 逻辑
    如果 索引（idx） 小于 0 或 索引 大于等于 字符串常量计数（g_str_count），那么：返回 0
    返回 加载单字节（驻留字符串获取（idx）, 声明索引（ci））
#### 测试要点
1. 越界返回 0
...
#### 作用
判断驻留表中索引 索引（idx） 的字符串是否与字面量字符串 字面量（lit） 相等。越界或不等返回 0（假），相等返回 1（真）。
#### 逻辑
    如果 索引（idx） 小于 0 或 索引 大于等于 字符串常量计数（g_str_count），那么：返回 0
...
| 扩展.rodata 引用（grow_rodataref） | g_x86_rodataref_pos, g_x86_rodataref_ro | 64 | 64 |

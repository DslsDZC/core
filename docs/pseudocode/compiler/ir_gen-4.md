...
| 压入looplabels | push_loop_labels | 生成表达式（gen_expr） |
| 弹出looplabels | pop_loop_labels | 生成表达式（gen_expr） |
...
| 函数信息访问器：ast节点 | fi_ast_node | IR 生成函数（ir_gen_func） |
...
| 强制 if-then 块 | force_if_thunk | 生成表达式（gen_expr） |
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 代码块表达式（EXPR_BLOCK），那么：
    令 语句起始索引 = AST 访问器：变量甲（a）（ast_a）（节点）
    令 语句数量 = AST 访问器：变量乙（b）（ast_b）（节点）
    令 最后语句结果 = -1
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 条件表达式（EXPR_IF），那么：
    ...
    令 条件节点 = AST 访问器：变量甲（a）（ast_a）（节点）
    令 那么（then） 分支节点 = AST 访问器：变量乙（b）（ast_b）（节点）
    令 否则（else） 分支节点 = AST 访问器：变量丙（c）（ast_c）（节点）
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 条件循环表达式（EXPR_WHILE），那么：
    令 条件节点 = AST 访问器：变量甲（a）（ast_a）（节点）
    令 循环体节点 = AST 访问器：变量乙（b）（ast_b）（节点）
    令 循环头部标签 = 新建标签（new_label）（）
    令 循环体入口标签 = 新建标签（new_label）（）
    令 循环出口标签 = 新建标签（new_label）（）
    发射指令（emit）（IR_LABEL, -1, 循环头部标签, 0, 0, 0）
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 遍历循环表达式（EXPR_FOR），那么：
    令 循环变量名索引 = AST 访问器：变量甲（a）（ast_a）（节点）
    令 迭代器节点 = AST 访问器：变量乙（b）（ast_b）（节点）
    令 循环体节点 = AST 访问器：变量丙（c）（ast_c）（节点）
    令 起始值变量 = -1
    令 结束值变量 = -1
    （范围（range） 迭代器）
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 匹配表达式（EXPR_MATCH），那么：
    令 匹配目标表达式 = AST 访问器：变量甲（a）（ast_a）（节点）
    令 第一个分支节点 = AST 访问器：变量乙（b）（ast_b）（节点）
    ...
    循环：
        如果 当前分支节点 小于 0，那么：
            跳出循环
        令 分支模式节点 = AST 访问器：变量甲（a）（ast_a）（当前分支节点）
        令 分支体节点 = AST 访问器：变量乙（b）（ast_b）（当前分支节点）
        令 模式类别 = -1
        如果 分支模式节点 大于等于 0，那么：
        ...
        令 有下一分支 = 0
        如果 AST 访问器：变量丙（c）（ast_c）（当前分支节点） 大于等于 0，那么：
            令 有下一分支 = 1
        ...
        如果 模式类别 等于 枚举模式（EXPR_ENUMPAT），那么：
            令 子值数量 = AST 访问器：变量丙（c）（ast_c）（分支模式节点）
            令 字段扫描位置 = 0
            循环：
                ...
                令 子模式节点 = AST 访问器：变量乙（b）（ast_b）（分支模式节点） + 字段扫描位置
        ...
        令 当前分支节点 = AST 访问器：变量丙（c）（ast_c）（当前分支节点）
    发射指令（emit）（IR_LABEL, -1, 汇合标签, 0, 0, 0）
    返回 结果变量
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 变量声明表达式（EXPR_LET），那么：
    令 变量名索引 = AST 访问器：变量甲（a）（ast_a）（节点）
    令 类型标注节点 = AST 访问器：变量乙（b）（ast_b）（节点）
    令 初始值节点 = AST 访问器：变量丙（c）（ast_c）（节点）
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 让出表达式（EXPR_YIELD），那么：
    令 产出值节点 = AST 访问器：变量甲（a）（ast_a）（节点）
    令 产出值变量 = -1
    如果 产出值节点 大于等于 0，那么：
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 等待表达式（EXPR_AWAIT），那么：
    令 被等待值节点 = AST 访问器：变量甲（a）（ast_a）（节点）
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 返回表达式（EXPR_RETURN），那么：
    如果 AST 访问器：变量甲（a）（ast_a）（节点） 大于等于 0，那么：
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 枚举构造表达式（EXPR_ENUM_CONSTRUCTOR），那么：
    ...
    循环：
        ...
        令 参数链节点 = AST 访问器：变量乙（b）（ast_b）（参数链节点）
        令 参数索引 = 参数索引 + 1
    返回 枚举变量
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 结构构造表达式（EXPR_STRUCT），那么：
    令 结构体名索引 = AST 访问器：变量甲（a）（ast_a）（节点）
    令 结构体变量 = 新建 IR 变量（new_ir_var）（"结构（struct）", 类型信息：单元（TI_UNIT））
    发射指令（emit）（IR_ALLOC_STRUCT, 结构体变量, 0, 0, 结构体名索引, 0）
    令 字段索引 = 0
    令 字段节点序列 = AST 访问器：变量乙（b）（ast_b）（节点）
    循环：
        如果 字段索引 大于等于 AST 访问器：变量丙（c）（ast_c）（节点），那么：
            跳出循环
        如果 字段节点序列 大于等于 0，那么：
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 数组表达式（EXPR_ARRAY），那么：
    ...
    令 元素索引 = 0
    令 元素节点序列 = AST 访问器：变量甲（a）（ast_a）（节点）
    循环：
        如果 元素索引 大于等于 AST 访问器：变量乙（b）（ast_b）（节点），那么：
            跳出循环
        如果 元素节点序列 大于等于 0，那么：
...
如果 AST 访问器：类别（ast_kind）（节点） 等于 元组表达式（EXPR_TUPLE），那么：
    令 首个元素AST索引 = AST 访问器：变量甲（a）（ast_a）（节点）
    令 元素个数 = AST 访问器：变量乙（b）（ast_b）（节点）
...
令 函数名索引 = AST 访问器：变量甲（a）（ast_a）（函数AST节点）
令 第一个参数节点 = AST 访问器：变量乙（b）（ast_b）（函数AST节点）
令 参数总数 = AST 访问器：变量丙（c）（ast_c）（函数AST节点）
...
循环：
    ...
    如果 当前参数节点 小于 0，那么：
        跳出循环
    令 参数名索引 = AST 访问器：变量甲（a）（ast_a）（当前参数节点）
...
循环：
    ...
    如果 AST 访问器：变量甲（a）（ast_a）（当前声明节点） 等于 名字索引，那么：
        令 值节点 = AST 访问器：变量丙（c）（ast_c）（当前声明节点）
        如果 值节点 大于等于 0，那么：
            令 值类别 = AST 访问器：类别（ast_kind）（值节点）
            如果 值类别 等于 整数字面量表达式（EXPR_INT） 或 值类别 等于 布尔字面量表达式（EXPR_BOOL），那么：
                返回 AST 访问器：整数值（ast_int_val）（值节点）
            如果 值类别 等于 一元运算表达式（EXPR_UNARY） 且 AST 访问器：变量丙（c）（ast_c）（值节点） 等于 取负运算（UOP_NEG），那么：
                令 内部节点 = AST 访问器：变量甲（a）（ast_a）（值节点）
...
如果 节点类别 等于 调用表达式（EXPR_CALL），那么：
    令 被调用函数节点 = AST 访问器：变量甲（a）（ast_a）（节点）
...
如果 节点类别 等于 代码块表达式（EXPR_BLOCK），那么：
    令 语句起始位置 = AST 访问器：变量甲（a）（ast_a）（节点）
    令 语句数量 = AST 访问器：变量乙（b）（ast_b）（节点）
    令 语句扫描位置 = 0
    循环：
...
否则如果 节点类别 等于 条件表达式（EXPR_IF） 或 等于 无限循环表达式（EXPR_LOOP） 或 等于 条件循环表达式（EXPR_WHILE） 或 等于 非安全块表达式（EXPR_UNSAFE），那么：
    ...
    如果 节点类别 等于 条件表达式（EXPR_IF），那么：
        如果 AST 访问器：变量乙（b）（ast_b）（节点） 大于等于 0，那么：
            AST 修补节点（ast_patch_node）（第二子节点访问器（ast_b）（节点）, 替换源前缀, 替换目标前缀）
        如果 AST 访问器：变量丙（c）（ast_c）（节点） 大于等于 0，那么：
...
否则如果 节点类别 等于 调用表达式（EXPR_CALL） 或 等于 枚举构造表达式（EXPR_ENUM_CONSTRUCTOR），那么：
    如果 AST 访问器：变量甲（a）（ast_a）（节点） 大于等于 0，那么：
        AST 修补节点（ast_patch_node）（第一子节点访问器（ast_a）（节点）, 替换源前缀, 替换目标前缀）
    令 参数链 = AST 访问器：变量乙（b）（ast_b）（节点）
    令 参数个数 = AST 访问器：变量丙（c）（ast_c）（节点）
    令 参数扫描位置 = 0
    循环：
...
否则如果 节点类别 等于 匹配表达式（EXPR_MATCH），那么：
    如果 AST 访问器：变量甲（a）（ast_a）（节点） 大于等于 0，那么：
        AST 修补节点（ast_patch_node）（第一子节点访问器（ast_a）（节点）, 替换源前缀, 替换目标前缀）
    令 分支链 = AST 访问器：变量乙（b）（ast_b）（节点）
    循环：
        如果 分支链 小于 0，那么：
            跳出循环
        如果 AST 访问器：变量甲（a）（ast_a）（分支链） 大于等于 0，那么：
            AST 修补节点（ast_patch_node）（第一子节点访问器（ast_a）（分支链）, 替换源前缀, 替换目标前缀）
        如果 AST 访问器：变量乙（b）（ast_b）（分支链） 大于等于 0，那么：
            AST 修补节点（ast_patch_node）（第二子节点访问器（ast_b）（分支链）, 替换源前缀, 替换目标前缀）
        令 分支链 = AST 访问器：变量丙（c）（ast_c）（分支链）
...
否则如果 节点类别 等于 结构构造表达式（EXPR_STRUCT），那么：
    令 字段链 = AST 访问器：变量乙（b）（ast_b）（节点）
    令 字段个数 = AST 访问器：变量丙（c）（ast_c）（节点）
    令 字段扫描位置 = 0
    循环：
...
否则如果 节点类别 等于 数组表达式（EXPR_ARRAY） 或 等于 元组表达式（EXPR_TUPLE），那么：
    令 元素链 = AST 访问器：变量乙（b）（ast_b）（节点）
    令 元素个数 = AST 访问器：变量丙（c）（ast_c）（节点）
    令 元素扫描位置 = 0
    循环：
...
3. 递归遍历覆盖所有 AST 节点类别（共 13 个 否则（else）-如果（if） 分支）
4. 无效节点（小于 0）立即返回
...
循环：
    ...
    如果 当前参数节点 小于 0，那么：
        跳出循环
    令 参数名驻留索引 = AST 访问器：变量甲（a）（ast_a）（当前参数节点）
...
（名字——逐个字符哈希）
令 函数名驻留索引 = AST 访问器：变量甲（a）（ast_a）（函数AST节点）
...
（参数类型——逐个参数的类型值取模后叠加）
令 参数节点 = AST 访问器：变量乙（b）（ast_b）（函数AST节点）
令 参数总数 = AST 访问器：变量丙（c）（ast_c）（函数AST节点）
令 参数扫描位置 = 0
循环：

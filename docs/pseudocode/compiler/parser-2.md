    ...
    循环（当 真 时）：
        ...
        如果 词法单元类别（tok_k（当前词法单元索引）） 等于 左圆括号（T_LPAREN），那么：
            ...
            如果 名称索引 大于等于 0，那么：
                ...
                令 首字符（c） = 取字符串第 次数（N） 字符（get_char（名称, 0））
...
函数 判断首字母大写（is_upper_first）（s） -> 布尔
    令 首字符（c） = 取字符串第 次数（N） 字符（get_char）（状态（s）, 0）
    如果 字符串比较（str_cmp）（首字符, "左操作数（A）"） 大于等于 0 且 字符串比较（str_cmp）（首字符, "Z"） 小于等于 0，那么：返回 真
    返回 假
`
### 测试要点
1. "Foo" 返回真；"foo" 返回假
2. 空字符串行为取决于 取字符串第 次数（N） 字符（get_char）（"", 0） 的返回值

## 函数 解析基本表达式（parse_primary）
### 作用
解析所有可能作为后缀链起点的基本表达式。支持的种类：
- 整数字面量（整数字面量（T_INT） 及带位宽后缀 8位整数字面量（T_INT_I8）~64位无符号整数字面量（T_INT_U64））：构造 整数字面量表达式（EXPR_INT），宽度信息写入 数据（data） 字段
- 浮点字面量（浮点字面量（T_FLOAT） / 32位浮点字面量（T_FLOAT_F32） / 64位浮点字面量（T_FLOAT_F64））：构造 浮点字面量表达式（EXPR_FLOAT），宽度写入 数据（data）
...
函数 解析基本表达式（parse_primary）（） -> 整数
    令 当前词法单元索引（t） = 当前词法单元（cur_tok（））
    -- 整数字面量（含带位宽后缀的类型化整数）
    如果 词法单元类别（tok_k（当前词法单元索引）） 等于 整数字面量（T_INT） 或 （词法单元类别（tok_k（当前词法单元索引）） 大于等于 8位整数字面量（T_INT_I8） 且 词法单元类别（tok_k（当前词法单元索引）） 小于等于 64位无符号整数字面量（T_INT_U64）），那么：
        前进词法单元（advance_tok）（）
        令 类别（kn） = 词法单元类别（tok_k（当前词法单元索引））
        令 宽度（w） = 0（可变）
        如果 类别 等于 8位整数字面量（T_INT_I8），那么：宽度 = 8位有符号宽度（W_I8）
        否则如果 类别 等于 16位整数字面量（T_INT_I16），那么：宽度 = 16位有符号宽度（W_I16）
        否则如果 类别 等于 32位整数字面量（T_INT_I32），那么：宽度 = 32位有符号宽度（W_I32）
        否则如果 类别 等于 64位整数字面量（T_INT_I64），那么：宽度 = 64位有符号宽度（W_I64）
        否则如果 类别 等于 8位无符号整数字面量（T_INT_U8），那么：宽度 = 8位无符号宽度（W_U8）
        否则如果 类别 等于 16位无符号整数字面量（T_INT_U16），那么：宽度 = 16位无符号宽度（W_U16）
        否则如果 类别 等于 32位无符号整数字面量（T_INT_U32），那么：宽度 = 32位无符号宽度（W_U32）
        否则如果 类别 等于 64位无符号整数字面量（T_INT_U64），那么：宽度 = 64位无符号宽度（W_U64）
        返回 分配 AST 节点（alloc_node）（整数字面量表达式（EXPR_INT）, 0, 0, 0, 词法单元整数值（tok_iv（当前词法单元索引））, 整数类型（TY_INT）, 宽度, 词法单元行号（tok_ln（当前词法单元索引））, 词法单元列号（tok_cl（当前词法单元索引）））
    -- 浮点字面量
    如果 词法单元类别（tok_k（当前词法单元索引）） 等于 浮点字面量（T_FLOAT） 或 词法单元类别（tok_k（当前词法单元索引）） 等于 32位浮点字面量（T_FLOAT_F32） 或 词法单元类别（tok_k（当前词法单元索引）） 等于 64位浮点字面量（T_FLOAT_F64），那么：
        前进词法单元（advance_tok）（）
        令 类别（kn） = 词法单元类别（tok_k（当前词法单元索引））
        令 宽度（w） = 0（可变）
        如果 类别 等于 32位浮点字面量（T_FLOAT_F32），那么：宽度 = 32位浮点宽度（W_F32）
        否则如果 类别 等于 64位浮点字面量（T_FLOAT_F64），那么：宽度 = 64位浮点宽度（W_F64）
        返回 分配 AST 节点（alloc_node）（浮点字面量表达式（EXPR_FLOAT）, 0, 0, 0, 词法单元整数值（tok_iv（当前词法单元索引））, 浮点类型（TY_FLOAT）, 宽度, 词法单元行号（tok_ln（当前词法单元索引））, 词法单元列号（tok_cl（当前词法单元索引）））
    -- 字符串字面量
...
前进词法单元（advance_tok）（） -- 跳过 」
    ...
    如果 词法单元类别（tok_k（当前词法单元索引）） 等于 协程启动关键字（T_GO），那么：
        ...
        -- 尝试范围模式：协程启动（go） 变量（枚举）（var） 开始（start）..结束（end） 表达式（expr）
...
- `变量甲（x） : 类型（枚举）（type） ...`（标识符（T_IDENT） 后跟 冒号（T_COLON））
- `变量甲（a）, 变量乙（b）, ..., 变量乙（z） : 类型（枚举）（type） ...` 或 `变量甲, 变量乙, ..., 变量乙 := ...`（批量声明，任意个 标识符（T_IDENT） 以逗号分隔，最后一个 标识符 后跟 `:` 或 `:=`）
### 逻辑
`
函数 判断新变量声明（is_new_var_decl）（） -> 布尔
    ...
    -- 模式二：变量甲（x） : 类型（枚举）（type） ...
    如果 词法单元类别（tok_k（位置索引 + 1）） 等于 冒号（T_COLON），那么：返回 真
    -- 模式三：变量甲（a）, 变量乙（b）, ..., 变量乙（z） : 类型（枚举）（type） ... 或 变量甲, 变量乙, ..., 变量乙 := ...（批量声明）
...
1. 解析名字列表（逗号分隔的标识符序列）
2. 若为 `:=` 则解析右侧表达式列表
3. 若为 `: 类型（枚举）（type）` 则解析类型，可选标签（可变（mut）, 公开（pub）, 插件标签），再解析 `= expr_list`
...
### 作用
解析 `遍历（for） 变量（枚举）（var） := 范围表达式（range_expr） 「 函数体（AST 节点）（body） 」` 迭代表达式。Core 仅支持 遍历...范围（range） 风格，不支持 C 风格三要素。若 `:=` 后非 `「` 则跳过词法单元直到遇到 `「` 或 文件结束标记（EOF），避免错误级联。
### 逻辑
`
函数 解析 遍历（for） 表达式（parse_for_expr）（） -> 整数
    ...
    令 循环体（body） = -1（可变）
-- Core 仅支持 遍历（for） 变量（枚举）（var） := 范围表达式（range_expr） 「 函数体（AST 节点）（body） 」；非 「 则跳过后续词法单元防止级联错误
...
函数 解析模式（parse_pattern）（） -> 整数
    ...
    -- 整数字面量模式
    如果 词法单元类别（tok_k（起始词法单元索引）） 等于 整数字面量（T_INT） 或 （词法单元类别（tok_k（起始词法单元索引）） 大于等于 8位整数字面量（T_INT_I8） 且 词法单元类别（tok_k（起始词法单元索引）） 小于等于 64位无符号整数字面量（T_INT_U64）），那么：
        前进词法单元（advance_tok）（）
        令 类别（kn） = 词法单元类别（tok_k（起始词法单元索引））
        令 宽度（w） = 0（可变）
        如果 类别 等于 8位整数字面量（T_INT_I8），那么：宽度 = 8位有符号宽度（W_I8）
        否则如果 类别 等于 16位整数字面量（T_INT_I16），那么：宽度 = 16位有符号宽度（W_I16）
        否则如果 类别 等于 32位整数字面量（T_INT_I32），那么：宽度 = 32位有符号宽度（W_I32）
        ...
        否则如果 类别 等于 32位无符号整数字面量（T_INT_U32），那么：宽度 = 32位无符号宽度（W_U32）
        否则如果 类别 等于 64位无符号整数字面量（T_INT_U64），那么：宽度 = 64位无符号宽度（W_U64）
        返回 分配 AST 节点（alloc_node）（整数字面量表达式（EXPR_INT）, 0, 0, 0, 词法单元整数值（tok_iv（起始词法单元索引））, 整数类型（TY_INT）, 宽度, 词法单元行号（tok_ln（起始词法单元索引））, 词法单元列号（tok_cl（起始词法单元索引）））
    -- 字符串字面量模式

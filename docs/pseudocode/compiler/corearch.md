# corearch.cr 伪代码
> 源文件：src/compiler/corearch.cr（149 行）
...
函数 （corearch） 主入口（corearch_main）（） -> 整数：
    ...
    令 文件描述符（fd） = 三参系统调用（syscall3）（2，源路径，0，0）    // 打开（open）：只读（O_RDONLY）
    如果 文件描述符 小于 0，那么：
        输出字符串（"error: cannot open "）
        输出行（源路径）
        返回 1

    令 文件大小（fsize） = 三参系统调用（syscall3）（8，文件描述符，0，2）    // 文件定位（lseek）：移到文件末尾（SEEK_END）
    三参系统调用（syscall3）（8，文件描述符，0，0）    // 文件定位（lseek）：回到文件开头（SEEK_SET）

    令 读取缓冲区（buf） = 分配内存（文件大小 + 1）
    令 读取字节数（nread） = 三参系统调用（syscall3）（0，文件描述符，读取缓冲区，文件大小）    // 读取（read）
    三参系统调用（syscall3）（3，文件描述符，0，0）    // 关闭（close）
    ...
    如果 发射共享库标记 不等于 0，那么：
        ...
        写 16 位（w16）（ELF 输出缓冲区，16，3）    // 修改 入口类型字段（e_type）为 共享对象类型（ET_DYN）（3）

        令 文件描述符 = 三参系统调用（syscall3）（2，输出字符串路径，577，420）    // 打开（open）：O_WRONLY|O_CREAT|O_TRUNC，0644
        如果 文件描述符 小于 0，那么：
            输出字符串（"error: cannot 写入（write） "）
            输出行（输出字符串路径）
            返回 1

        三参系统调用（syscall3）（1，文件描述符，ELF 输出缓冲区，ELF 总大小）    // 写入（write）
        三参系统调用（syscall3）（3，文件描述符，0，0）    // 关闭（close）
...
8. 纯静态（无 --link 且非 --shared）模式设置 x86 是否需要发射运行时桩标记 = 1

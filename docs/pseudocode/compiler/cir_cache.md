...
| 数据流图各函数节点起始索引 | g_df_func_node_start | 保存 CIR 缓存 |
| 数据流图各函数节点计数 | g_df_func_node_count | 保存 CIR 缓存 |
...
## 函数 保存 数据流图（CIR） 缓存（save_cir_cache）
### 作用
将一个函数的数据流图（DFG）状态序列化保存到 .数据流图（cir） 缓存文件中。先打开/创建文件（O_WRONLY|O_CREAT|O_TRUNC），计算函数指纹和签名指纹，将头信息（魔数、版本、指纹）、变量表、节点表、边表（v5 格式含 种类（kind） 字段）、指令表、字符串常量逐一写入内存缓冲区，最后通过单次系统调用写入文件。缓冲区策略（先序列化到内存再一次性写入）避免逐个字段的 写入（write） 调用产生数百万次系统调用。
### 逻辑
    ...
    令 节点起始（node_start） = 读 64 位（r64）（数据流图各函数节点起始索引（g_df_func_node_start），函数索引 * 8）
    令 节点计数（node_count） = 读 64 位（r64）（数据流图各函数节点计数（g_df_func_node_count），函数索引 * 8）
    ...
    循环（当 大小遍历索引 小于 IR 字符串常量计数（g_ir_str_const_count） 时）：
        ...
        令 大小遍历索引 = 大小遍历索引 + 1
    如果 总大小（total_size） 大于 数据流图（CIR） 写缓冲区容量（g_cir_write_cap），那么：
        令 新容量（new_cap） = 数据流图（CIR） 写缓冲区容量（g_cir_write_cap） * 2
        如果 新容量（new_cap） 小于 4096，那么：令 新容量（new_cap） = 4096
        如果 新容量（new_cap） 小于 总大小（total_size），那么：令 新容量（new_cap） = 总大小（total_size）
        令 数据流图（CIR） 写缓冲区（g_cir_write_buf） = 分配（alloc）（new_cap）
        令 数据流图（CIR） 写缓冲区容量（g_cir_write_cap） = 新容量（new_cap）
    令 数据流图（CIR） 写缓冲区位置（g_cir_write_pos） = 0
    写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），数据流图 缓存魔数（CIR_CACHE_MAGIC））
    写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），数据流图 缓存版本（CIR_CACHE_VER））
    写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），函数体指纹（fp））
    写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），签名指纹（sig））
    写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），名称长度（name_len））
    写入文件描述符（write_fd）（文件描述符（fd），函数名（name），名称长度（name_len））
    写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），变量计数（var_count））
    令 变量索引（vi） = 0（可变）
    循环（当 变量索引 小于 变量计数（var_count） 时）：
        令 变量全局索引（v_idx） = 变量起始（var_start） + 变量索引
        写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），IR 变量访问器：名称（irv_name）（v_idx））
        写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），IR 变量访问器：ID（irv_id）（v_idx））
        写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），IR 变量访问器：类型（irv_type）（v_idx））
        令 变量索引 = 变量索引 + 1
    写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），节点计数（node_count））
    令 节点遍历索引（ni2） = 0（可变）
    ...
    循环（当 指令索引 小于 指令计数（instr_count） 时）：
        ...
        写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），IR 指令访问器：操作码（iri_op）（inst））
        写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），IR 指令访问器：目标（iri_dest）（inst））
        写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），IR 指令访问器：操作数1（iri_s1）（inst））
        写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），IR 指令访问器：操作数2（iri_s2）（inst））
        写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），IR 指令访问器：操作数3（iri_s3）（inst））
        写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），IR 指令访问器：类型类别（iri_tk）（inst））
        令 指令索引 = 指令索引 + 1
    写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），IR 字符串常量计数（g_ir_str_const_count））
    令 字符串索引（si） = 0（可变）
    循环（当 字符串索引 小于 IR 字符串常量计数（g_ir_str_const_count） 时）：
        ...
        写 64 位（数据流图（CIR） 缓存）（w64_cir）（文件描述符（fd），字符串长度（sl））
        令 变量索引（vi） = 0
        循环（当 变量索引 小于 字符串长度（sl） 时）：
            写单字节（数据流图（CIR） 缓存）（w8_cir）（文件描述符（fd），无符号单字节读取（load8）（常量字符串（s），变量索引））
            令 变量索引 = 变量索引 + 1
        令 字符串索引 = 字符串索引 + 1
    令 已写入（written） = 系统调用3（syscall3）（1，文件描述符（fd），数据流图（CIR） 写缓冲区（g_cir_write_buf），数据流图 写缓冲区位置（g_cir_write_pos））
    系统调用3（syscall3）（3，文件描述符（fd），0，0）
    如果 已写入（written） 不等于 数据流图（CIR） 写缓冲区位置（g_cir_write_pos），那么：返回 -1
    返回 0
### 测试要点
...
## 函数 写 64 位（数据流图（CIR） 缓存）（w64_cir）
### 作用
将一个 64 位整数写入序列化缓冲区（g_cir_write_buf）的当前位置，然后将写位置前进 8 字节。文件描述符（fd） 参数被保留以兼容旧接口但未使用。
### 逻辑
    写 64 位（w64）（数据流图（CIR） 写缓冲区（g_cir_write_buf），数据流图 写缓冲区位置（g_cir_write_pos），值（val））
    令 数据流图（CIR） 写缓冲区位置（g_cir_write_pos） = 数据流图 写缓冲区位置（g_cir_write_pos） + 8
### 测试要点
1. 连续写入多个值后，写位置正确累加（每次 + 8）
2. 写入值可随后用 读 64 位（r64） 在同一位置读回且一致

## 函数 写单字节（数据流图（CIR） 缓存）（w8_cir）
### 作用
将一个字节写入序列化缓冲区的当前位置，然后将写位置前进 1 字节。
### 逻辑
    写单字节字节存储（store8）（数据流图（CIR） 写缓冲区（g_cir_write_buf），数据流图 写缓冲区位置（g_cir_write_pos），值（val））
    令 数据流图（CIR） 写缓冲区位置（g_cir_write_pos） = 数据流图 写缓冲区位置（g_cir_write_pos） + 1
### 测试要点
1. 连续写入多个字节后，写位置正确累加（每次 + 1）
2. 和 写 64 位（数据流图（CIR） 缓存）（w64_cir） 交替使用时写位置保持正确
...
### 逻辑
    ...
    循环（当 写入位置 小于 长度（len） 时）：
        写单字节字节存储（store8）（数据流图（CIR） 写缓冲区（g_cir_write_buf），数据流图 写缓冲区位置（g_cir_write_pos），无符号单字节读取（load8）（数据（data），写入位置））
        令 数据流图（CIR） 写缓冲区位置（g_cir_write_pos） = 数据流图 写缓冲区位置（g_cir_write_pos） + 1
        令 写入位置 = 写入位置 + 1
### 测试要点
...
## 函数 加载 数据流图（CIR） 缓存（load_cir_cache）
### 作用
从 .数据流图（cir） 缓存文件中恢复一个函数的数据流图（DFG）状态。先读取整个文件到内存，验证魔数、版本（必须为 v5）、函数体指纹和签名指纹——若指纹与当前 AST 不匹配（函数体或签名已更改）则缓存失效，返回 -1。验证通过后，将缓存的变量、节点（含 节点（node）-region 映射和变量生产者映射）、边（kind）、指令和字符串常量逐一恢复到全局数组中。恢复后的节点直接追加到现有节点数组末尾（base_node），其第一边索引和边计数字段也被还原，无需重新计算边链表。
### 逻辑
    ...
    令 位置（pos） = 位置（位置） + 8
    如果 魔数（magic） 不等于 数据流图（CIR） 缓存魔数（CIR_CACHE_MAGIC），那么：返回 -1
    令 版本（ver） = 读 64 位（r64）（文件数据（data），位置（pos））
    令 位置（pos） = 位置（位置） + 8
    如果 版本（ver） 不等于 数据流图（CIR） 缓存版本（CIR_CACHE_VER），那么：返回 -1
...
2. 魔数不匹配时返回 -1
3. 版本号不等于 数据流图（CIR） 缓存版本（CIR_CACHE_VER）（5）时返回 -1（旧版本缓存被拒绝）
...
## 函数 创建 数据流图（CIR） 缓存目录（make_cir_cache_dir）
### 作用
创建 .数据流图（cir） 缓存所需的目录层级（.核心（core）/、.核心/cache/、.核心/cache/数据流图/）。使用 Linux 建目录（mkdir） 系统调用（83），权限 0700（448 = 0x1C0）。对已存在的目录忽略 已存在错误码（EEXIST） 错误——系统调用本身的返回值不被检查，目录已存在时 建目录 返回 -已存在错误码 但不影响后续操作。
### 逻辑
    系统调用3（syscall3）（83，".核心（core）"，448，0）
    系统调用3（syscall3）（83，".核心（core）/cache"，448，0）
    系统调用3（syscall3）（83，".核心（core）/cache/数据流图（cir）"，448，0）
### 测试要点

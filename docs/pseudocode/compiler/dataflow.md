...
## 函数 压入结构图（sg_push）
### 作用
在数据流图中打开一个新的嵌套 region（子图）。根据给定的类别（kind）创建结构图（SG）记录，记录入口节点位置，查找并绑定父 region（最内层当前未关闭的 region），然后将自身设为当前最内层活跃 region。用于标记函数体、条件分支、循环体等嵌套结构的边界。
### 逻辑
...
### 作用
在数据流图中创建一个新节点，填写操作码、目标变量、源操作数和类型类别，并将其所属的 region 记录到节点-region 映射中。如果目标变量大于等于 0，则更新变量生产者映射（g_df_var_producer）记录此节点产生该变量。随后自动连接源操作数边（df_connect_srcs）和状态边（df_connect_state），返回新节点的 ID。
### 逻辑
...
### 作用
在数据流图中添加一条有向边，从源节点指向目标节点，类别为指定的边类别（0 = 数据边/定义-使用边，1 = 状态边/顺序约束）。采用链表头插法：新边插入到源节点的边链表头部，边计数加一。
### 逻辑
...
### 作用
添加一条数据边（定义-使用边），类别固定为 0。是对数据流图：添加带类别边的便捷封装。
### 逻辑
    数据流图：添加带类别边（df_add_edge_kind）（源节点 ID（from_id），目标节点 ID（to_id），0）
### 测试要点
1. 等价于调用数据流图：添加带类别边且类别为 0
...
### 逻辑
    ...
    如果 是否副作用 不等于 0，那么：
        如果 最后一个状态节点（g_last_state_node） 大于等于 0，那么：数据流图：添加带类别边（df_add_edge_kind）（最后一个状态节点（g_last_state_node），节点 ID（node_id），1）
...
### 逻辑
    ...
    如果 变量索引（var_idx） 大于等于 数据流图数组容量（g_df_cap），那么：扩展数据流数组（grow_df_arrays）（变量索引（var_idx） + 1）
...
### 逻辑
    ...
    如果 操作码（opcode） 等于 IR 返回（IR_RETURN），那么：
        如果 源操作数1（s1） 大于等于 0，那么：数据流图：使用变量（df_use_var）（节点 ID（node_id），源操作数1（s1））
        返回
...
### 逻辑
    ...
    如果 新容量（nc） 小于 128，那么：令 新容量（名称计数） = 128
    如果 新容量（nc） 小于 所需容量（needed），那么：令 新容量（名称计数） = 所需容量（需求） + 128
...
### 作用
遍历所有数据流边，对每条非状态边（种类（kind） != 0 的边），统计其源节点的目标变量（dest）被使用的次数，记录到变量使用计数数组（g_var_use_count）中。状态边（种类 != 0）不参与使用计数（它们不是数据消费者）。
### 逻辑
    令 边索引（ei） = 0（可变）
...
### 作用
将数据流图中的节点按创建顺序（即 AST 遍历顺序，已是有效的拓扑排序）线性化到 IR 指令数组（g_ir_instrs）中，重置 IR 指令计数后逐一拷贝节点的操作码、目标变量、源操作数和类型类别。完成后更新每个函数的指令起始/计数以匹配新的 IR 指令数组布局，并调用计算变量使用次数（compute_usage_counts）供后续优化趟使用。
### 逻辑
...
2. 节点索引值与指令索引值一一对应（一对一映射），确保函数边界在数据流图和线性指令数组之间一致
3. 函数计数（g_ir_func_count）为 0 时，第二个循环不执行（不崩溃）
4. 降级后的 IR 指令数组全部字段与数据流节点一致（操作码、目标、源操作数、类型类别）

## 函数 数据流图：开始函数（df_begin_func）
### 作用
在数据流图中标记一个函数的起始边界：记录当前节点计数作为该函数的起始节点位置，为该函数压入一个函数类型 region，并重置状态依赖图状态链（每个函数的状态链独立）。
### 逻辑
...
### 作用
在数据流图中标记一个函数的结束边界：计算该函数的节点计数（当前节点数减去起始位置），然后弹出对应的SG 函数类型常量（SG_FUNC）region。
### 逻辑
    如果 函数索引（func_idx） 大于等于 0，那么：
...
### 作用
将整个数据流图导出为可视化图格式（Graphviz DOT）的字符串。输出包括：region 子图簇（每个结构图一个 DOT cluster，含节点列表）、节点定义（操作码名加上变量名作为标签）、边定义（状态边用红色虚线以突出顺序约束）。用于调试可视化数据流图的结构。
### 逻辑
    令 DOT 字符串 = 有向图声明开始 + 换行（DOT 格式头）（可变）
    令 DOT 字符串追加布局方向从上到下 + 换行（DOT 布局声明）
    令 结构图索引 = 0（可变）
    循环（当 结构图索引 小于 结构图计数（g_sg_count） 时）：
        令 结构图类别 = 读 64 位（r64）（结构图数组（g_sgs），结构图索引 * SG 条目大小（ESZ_SG） + SG 条目类别偏移（OFF_SG_KIND））
        令 结构图名称 = 区域默认名称（可变）
        如果 结构图类别 等于 SG 条件分支类型常量（SG_IF），那么：令 结构图名称设为条件分支名称
        如果 结构图类别 等于 SG 循环类型常量（SG_LOOP），那么：令 结构图名称设为循环名称
        如果 结构图类别 等于 SG 计数循环类型常量（SG_FOR），那么：令 结构图名称设为 计数循环名称
        如果 结构图类别 等于 SG 流程类型常量（SG_FLOW），那么：令 结构图名称设为流程名称
        如果 结构图类别 等于 SG 不安全类型常量（SG_UNSAFE），那么：令 结构图名称设为不安全名称
        令 DOT 字符串追加子图簇声明行（前缀 + 名称 + 编号 + 标签定义 + 换行）
        令 节点起始 = 读 64 位（r64）（结构图数组（g_sgs），结构图索引 * SG 条目大小（ESZ_SG） + SG 条目节点起始偏移（OFF_SG_NSTART））
        令 节点结束 = 读 64 位（r64）（结构图数组（g_sgs），结构图索引 * SG 条目大小（ESZ_SG） + SG 条目出口偏移（OFF_SG_EXIT））
        如果 节点结束 大于等于 0，那么：
            令 节点遍历索引 = 节点起始（可变）
            循环（当 节点遍历索引 小于 节点结束 时）：
                令 DOT 字符串追加节点编号声明 + 换行（格式为前缀加编号加换行）
                令 节点遍历索引 = 节点遍历索引 + 1
        令 DOT 字符串追加子图簇结束标记 + 换行（DOT 簇结束）
        令 结构图索引 = 结构图索引 + 1
    令 节点索引 = 0（可变）
    循环（当 节点索引 小于 数据流节点计数（g_df_node_count） 时）：
        令 节点操作码 = 读 64 位（r64）（数据流节点数组（g_df_nodes），节点索引 * 数据流节点条目大小（ESZ_DFNODE） + 数据流节点操作码偏移（OFF_DF_OPCODE））
        令 节点目标变量 = 读 64 位（r64）（数据流节点数组（g_df_nodes），节点索引 * 数据流节点条目大小（ESZ_DFNODE） + 数据流节点目标偏移（OFF_DF_DEST））
        令 节点源3 = 读 64 位（r64）（数据流节点数组（g_df_nodes），节点索引 * 数据流节点条目大小（ESZ_DFNODE） + 数据流节点源3偏移（OFF_DF_S3））
        令 标签 = 数据流图：操作码名称（df_opcode_name）（节点操作码，节点源3）（可变）
        如果 节点目标变量 大于等于 0，那么：
            令 变量名 = 获取 IR 变量名（get_ir_var_name）（节点目标变量）
            如果 字符串长度（str_len）（变量名） 大于 0，那么：
                令 标签 = 变量名 + 冒号 + 标签
        令 DOT 字符串追加节点标签定义行（前缀 + 编号 + 左方括号 + 标签键值 + 矩形形状键值 + 右方括号 + 换行）
        令 节点索引 = 节点索引 + 1
    令 边索引 = 0（可变）
    循环（当 边索引 小于 数据流边计数（g_df_edge_count） 时）：
        令 边源节点 = 读 64 位（r64）（数据流边数组（g_df_edges），边索引 * 数据流边条目大小（ESZ_DFEDGE） + 数据流边源节点偏移（OFF_DFE_FROM））
        令 边目标节点 = 读 64 位（r64）（数据流边数组（g_df_edges），边索引 * 数据流边条目大小（ESZ_DFEDGE） + 数据流边目标节点偏移（OFF_DFE_TO））
        如果 读 64 位（r64）（数据流边数组（g_df_edges），边索引 * 数据流边条目大小（ESZ_DFEDGE） + 数据流边类别偏移（OFF_DFE_KIND）） 不等于 0，那么：
            令 DOT 字符串追加带风格边声明（前缀 + 源编号 + 箭头 + 目标编号 + 左方括号 + 虚线样式键值 + 红色颜色键值 + 右方括号 + 换行）
        否则：
            令 DOT 字符串追加普通边声明（前缀 + 源编号 + 箭头 + 目标编号 + 换行）
        令 边索引 = 边索引 + 1
    令 DOT 字符串追加图定义结束标记 + 换行（DOT 格式尾）
    返回 DOT 字符串
### 测试要点
...
### 逻辑
    ...
    如果 操作码（opcode） 等于 IR 返回（IR_RETURN），那么：返回 返回名称（"返回（return）"）
    如果 操作码（opcode） 等于 IR 分配（IR_ALLOC），那么：返回 分配名称（"分配函数（alloc）"）
    ...
    如果 操作码（opcode） 等于 IR 引用（IR_REF），那么：返回 引用名称（"引用（ref）"）
    ...
    如果 操作码（opcode） 等于 IR 让出（IR_YIELD），那么：返回 让出名称（"让出（yield）"）

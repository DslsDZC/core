...
| 映射 | 实现 | 字节寻址、区域、Arena |
| 格 | 内存模型层 = 中间存在空间 | 图 → 格 → 二进制 三层链的第二层（本文件 = 格层内容；与「格（Lattice）层内代数」区分，见 memory-model-capability-lattice.md §四 4.4） |
...
> **2026-08-26 定位**：本文件是三层映射链（图 → 格 → 二进制，见 `docs/memory-model-capability-lattice.md` §四）中「格」层的内容——缓存语义 = 格层的语义本体（如何存在），经典映射 = 格在字节机器上的实例（如何实现）。图的职责（关系空间：非因果/模糊/超图灵）见 `docs/dataflow-design.md` §8——超图灵性属于图，格层只需提供足够一般的状态模型；二进制职责见各后端。
